...
class Preprocessor:
    ...
    def process_input(self, file_tuples):
        ...
        for ftuple in file_tuples:
            ...
            print(ip_addr)
            df_net = df_net[(df_net['ip.proto'] == 17) & (df_net['ip.dst'] == ip_addr)]
            ...
            if df_net.shape[0] == 0:
                idx += 1
                continue
            src_df = df_net.groupby('ip.src').agg({'length': sum, 'rtp.p_type': 'count'}).reset_index().sort_values(by='length', ascending=False).head(1)['ip.src']
            if len(src_df) == 0:
                idx += 1
                continue
            src = src_df.iloc[0]            
            ...
            df_merged = pd.merge(df, df_webrtc,
                                 left_on='et', right_on="ts")
            print(df_merged.head(5)[['framesReceivedPerSecond', 'bitrate', 'frame_jitter', 'et']])
            feature_file = csv_file[:-4]+f'_rtp_ml_{self.vca}_{self.dataset}.csv'
            df_merged.to_csv(feature_file, index=False)
            idx += 1

if __name__ == '__main__':
    dpath = '/home/taveesh/Documents/Projects/vca-qoe-inference/data/IMC_Lab_data'
    bname = basename(dpath)
    fp = FileProcessor(dpath, project_config['data_format'][bname])
