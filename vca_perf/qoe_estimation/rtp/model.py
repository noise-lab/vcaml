...
class RTPModel:
    ...
    def estimate(self, file_tuple):
        ...
        # dfr = df[(df['rtp.p_type'] == self.rtx_ptype[self.vca]) & (df['udp.length'] > 304)]
        # df = pd.concat([dfv, dfr], axis=0)
        df = df.sort_values(by=['frame.time_relative'])
        df['frame.time_relative'] = df['frame.time_relative'].astype(float)
        # print(df['rtp.p_type'].value_counts())
        df = df[(df['rtp.p_type'].isin([*self.config['video_ptype'][self.dataset][self.vca], *self.config['rtx_ptype'][self.dataset][self.vca]])) & (df['udp.length'] > 306)]
        ...
        # df_grp = df_grp[df_grp['frame_et'] - df_grp['frame_st'] <= 1]
        ...
        # df_merge['num_discarded'] = len(to_discard)
